...
## ensembel1 +ensemble2+ensemble3

pred<-merge(ensemble1,merge(ensemble2,ensemble3,by="CUS_ID"),by="CUS_ID")
nc <- ncol(pred)
nf <- 3
for (i in 1:6) {
  x <- 1
...
##ensemble4 + m1+m2
pred<-merge(ensemble4,merge(en3,en8,by="CUS_ID"),by="CUS_ID")
nc <- ncol(pred)
nf <- 3
for (i in 1:6) {
  x <- 1
...
names(pred) <- c("CUS_ID", "F20-", "F30", "F40+", "M20-", "M30", "M40+")
write.csv(pred,"7조-5차.csv", row.names = F)

a<-read.csv("7조-5차.csv")
head(a)
